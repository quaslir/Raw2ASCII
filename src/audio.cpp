...
void AudioPlayer::init() {
  ...
  frameIndex = av_find_best_stream(formatContext, AVMEDIA_TYPE_AUDIO, -1, -1,
                                   &audioCodec, 0);

  if (frameIndex >= 0) {
    codecContext = avcodec_alloc_context3(audioCodec);
    avcodec_parameters_to_context(codecContext,
                                  formatContext->streams[frameIndex]->codecpar);
  ...
  int res = swr_alloc_set_opts2(
      &swrContext, &layout, AV_SAMPLE_FMT_S16, 48000, &codecContext->ch_layout,
  ...
  ma_result initRes =
      ma_pcm_rb_init(ma_format_s16, 2, 48000 * 10, NULL, NULL, &rb);
...
void AudioPlayer::dataCallback(ma_device *pDev, void *output, const void *input,
  ...
  if (res == MA_SUCCESS && framesRead > 0) {
    std::memcpy(pOut, pBufferIn, framesRead * 2 * sizeof(int16_t));
    ma_pcm_rb_commit_read(&pAudio->rb, framesRead);

    if (framesRead < frames) {
      ma_uint32 dif = frames - framesRead;
      int16_t *pNextout = pOut + (framesRead * 2);

      void *pBufferInNext = nullptr;

      if (ma_pcm_rb_acquire_read(&pAudio->rb, &dif, &pBufferInNext) ==
          MA_SUCCESS) {
        std::memcpy(pNextout, pBufferInNext, dif * 2 * sizeof(int16_t));
...
void AudioPlayer::processAudioFrame(AVFrame *frame) {
  int samples = av_rescale_rnd(swr_get_delay(swrContext, frame->sample_rate) +
                                   frame->nb_samples,
...
void AudioPlayer::pushData(const int16_t *data, ma_uint32 count) {
  if (!data || count == 0)
    return;
